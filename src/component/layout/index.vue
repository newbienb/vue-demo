    ...
    <drawer :show.sync="drawerVisibility"
        ...
        <div class="left-sub-menu-bar">
          <router-link to="login"
            ...
            <i class="fa fa-envelope"
               style="color:#ffffff;font-size:2.0em;position: relative;top:0.18em" />
          </router-link>
        </div>
        <div class="left-sub-menu-bar">
          <a href="javascript:;"
             @click="drawerVisibility = false;showMenu = true">
      ...
      <view-box ref="viewBox"
        ...
        <x-header slot="header"
          ...
          <span v-if="route.path === '/' || route.path === '/home'"
                slot="overwrite-left"
          ...
          <span v-if="route.path !== '/edit'"
                slot="right">
            <x-icon @click.native="go('/edit')"
                    type="edit"
                    size="26"
                    style="fill:#fff;position:relative;top:-1px;left:-3px;"></x-icon>
          </span>
        </x-header>
        ...
        <tabbar slot="bottom">
          <tabbar-item @click.native="go('/home')"
                       :selected="route.path === '/' || route.path === '/home'">
            <i class="fa fa-home"
               style="font-size:28px;position: relative; top: 3px;"
               slot="icon" />
            <span slot="label">{{ $t('home') }}</span>
          </tabbar-item>
          <tabbar-item @click.native="go('/edit')"
                       :selected="route.path === '/edit'">
            <i class="fa fa-user"
               style="font-size:28px;position: relative; top: 3px;"
               slot="icon" />
            <span slot="label">
              <span class="tabbar-component-selected">{{ $t('edit.home') }}</span>
            </span>
          </tabbar-item>
          <tabbar-item @click.native="go('/user')"
                       :selected="route.path === '/login' || route.path === '/user'">
            <i class="fa fa-user"
               style="font-size:28px;position: relative; top: 3px;"
               slot="icon" />
            <span slot="label">
              <span class="tabbar-component-selected">{{ $t('user.module') }}</span>
            </span>
          </tabbar-item>
        </tabbar>
      </view-box>
  ...
  computed: {
    ...
    title() {
      ...
      if (this.route.path === '/edit') {
        return this.$t('edit.home')
      }
      if (this.route.path === '/login') {
        return this.$t('login')
      }
      if (this.route.path === '/user') {
        return this.$t('user.module')
      }
